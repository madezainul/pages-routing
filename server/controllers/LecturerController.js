...
class LecturerController {
  ...
  static delete(req, res) {
    ...
    lecturer
      ...
      .then((result) => {
        // if (result === 1) {
        //   res.json({
        //     message: `Lecturer with id ${id} has been deleted!`,
        //   });
        // } else {
        //   res.json({
        //     message: `Could not deleted id ${id}`,
        //   });
        // }
  ...
  static update(req, res) {
    ...
    .then(result => {
        result[0] === 1 ?
        res.json({
            message: `Lecturer with id ${id} has been updated!`
        }) :
        res.json({
            message: `Couldn't update Lecturer with id ${id}`
        })
    })
    .catch(err => {
        res,json(err)
